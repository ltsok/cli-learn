...
export class AlarmHistoryComponent {
  ...
  ngOnInit(): void {
    let me = this;
    //初始化告警条件--告警模板信息
    this.alarmHistoryTemplates.push({ "templateName": "不使用" });
    this.alarmHttpService.getAlarmTemplates().then(
      (value: any) => {
        value.forEach(element => {
          if (element.templateTypeId == AlarmTemplateType.HistoryAlarm) {
            this.alarmHistoryTemplates.push(element);
          }
        })
      },
  ...
  public pageChange(event: PageChangeEvent): void {
    ...
    this.alarmHttpService.queryHistoryAlms({
  ...
  confirmSaveAsTemplate() {
    ...
    if (this.saveTemplateName && this.saveTemplateName != "") {
      ...
      this.alarmHttpService.addHistoryAlmTemplate(almTemplate).then(
        (value) => {
          console.log(value);
          //完成另存为模板功能后，需重新查询更新当前可选模板信息
          this.alarmHistoryTemplates = [];
          this.alarmHistoryTemplates.push({ "templateName": "不使用" });
          this.alarmHttpService.getAlarmTemplates().then(
            (value: any) => {
              value.forEach(element => {
                if (element.templateTypeId == 2) {
                  this.alarmHistoryTemplates.push(element);
                }
              })
            },
  ...
  query() {
    ...
    this.alarmHttpService.queryHistoryAlms({
      ...
      (value: any) => {
        ...
        me.subscribeInfo = me.wsService.subscribeByQueryId("/alm/historyAlm", me.queryId, (retValue: any) => {
